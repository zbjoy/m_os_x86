...
static inline uint16_t inw(uint16_t port) // in 指令读端口
{
    uint16_t rv;
...
static inline void outb(uint16_t port, uint8_t data) // out 指令写端口
{
    __asm__ __volatile__("outb %[v], %[p]"::[p]"d"(port),[v]"a"(data));
}

static inline void cli(void) // 关中断
{
    __asm__ __volatile__("cli");
}

static inline void sti(void) // 开中断
{
